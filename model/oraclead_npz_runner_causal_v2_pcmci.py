# -*- coding: utf-8 -*-
"""
oraclead_npz_runner_causal_v2_pcmci.py

Same as causal_v2 but with PCMCI+ prior option replacing TE prior.
PCMCI+ provides more rigorous causal discovery:
  - Systematic conditional independence testing (not heuristic)
  - Handles contemporaneous + lagged causal effects
  - Removes spurious edges from confounders
...
def sample_intervention_pairs(tau_max: int, N: int, num_pairs: int, rng: np.random.Generator,
    ...
    to their gate strength — focusing intervention on edges that PCMCI+
    identified as causally important. This avoids wasting intervention
    on empty edges and measures the strength of real causal links more precisely.

    If te_prior_gate is None, falls back to uniform random sampling.
    """
    pairs = []

    if te_prior_gate is not None:
        # Sum gate over targets to get source importance per (tau, src)
        # te_prior_gate: [tau_max, N, N] -> source_importance: [tau_max, N]
        ...
        flat = source_importance.ravel()  # [tau_max * N]
        flat_sum = flat.sum()
...
def permutation_alignment_and_epoch_cls(model, X, x_true_next, base_abs_err_ref, edge_strength, args, rng):
    ...
    # PCMCI+ guided: prioritize edges that PCMCI+ identified as important
