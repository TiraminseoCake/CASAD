"""
oraclead_npz_runner_causal_v3.py

Architectural improvements over causal_v2:

  [V3-1] Replace N separate PerVarEncoder (LSTM) with SharedTemporalEncoder (1D Conv).
         Single shared encoder for all variables, with positional embeddings and
         residual conv blocks. Reduces parameter count from O(N*d^2) to O(d^2).

  [V3-2] Add CausalGNNLayer for message passing along learned causal edges.
         After encoding, variable embeddings exchange information along the
         causal graph edges (from edge_gate). This lets downstream predictions
         incorporate structural knowledge before MHSA.

  [V3-3] Replace N separate PerVarReconDecoder (LSTM) with SharedReconDecoder (Conv).
         Single shared decoder for all variables, further reducing parameters.

  [V3-4] Add learnable variable identity embeddings (var_embed).
         Since the encoder is shared, var_embed provides variable-specific bias.

  All other components (TE prior, scoring, losses, training loop, etc.) are
  identical to causal_v2.
"""
import argparse
import glob
import math
...
# [V3-1] SharedTemporalEncoder replaces N separate PerVarEncoder
class SharedTemporalEncoder(nn.Module):
    """Shared 1D Conv encoder for all variables.
    Input: [B*N, lag_win, 1], var_embed: [B*N, d] -> Output: [B*N, d]

    var_embed is injected INTO the conv input (not added after),
    so the same conv filters produce variable-specific outputs.
    """
    def __init__(self, d, num_layers=3, dropout=0.0):
        super().__init__()
        self.input_proj = nn.Linear(1, d)
        self.pos_embed = nn.Parameter(torch.randn(1, 50, d) * 0.02)  # max lag_win=50
        layers = []
        for _ in range(num_layers):
            layers.append(nn.Conv1d(d, d, kernel_size=3, padding=1))
    ...
    def forward(self, x, var_embed=None):
        # x: [B*N, T, 1], var_embed: [B*N, d] (optional)
        h = self.input_proj(x)  # [B*N, T, d]
        h = h + self.pos_embed[:, :h.size(1), :]
        # Inject var_embed into every timestep BEFORE conv
        # This makes the same conv produce variable-specific features
        if var_embed is not None:
            h = h + var_embed.unsqueeze(1)  # [B*N, T, d] + [B*N, 1, d] broadcast
        h = self.dropout(h)
        # Conv expects [B, C, T]
        h = h.transpose(1, 2)  # [B*N, d, T]
        h_conv = self.conv_stack(h)  # [B*N, d, T]
        h = h + h_conv  # residual
        h = h.transpose(1, 2)  # [B*N, T, d]
        return self.pool(h)  # [B*N, d]


# [V3-2] CausalGNNLayer for message passing along causal edges
class CausalGNNLayer(nn.Module):
    """GNN message passing along causal edges."""
    ...
    def forward(self, H, adj, mask_var=None):
        # H: [B, N, d], adj: [N, N] (src->tgt, adj[i,j] = edge i->j)
        # mask_var: int or None — if set, block messages FROM this variable
        #   This prevents GNN from "leaking" masked variable info to neighbors,
        #   which would defeat the purpose of intervention masking.
        if mask_var is not None:
            adj = adj.clone()
            adj[int(mask_var), :] = 0.0  # block all outgoing edges from masked var

        for layer in self.layers:
            msg = layer['msg_fn'](H)          # [B, N, d]
            agg = torch.einsum('bsd,st->btd', msg, adj)  # [B, N_tgt, d]
            H = H + self.dropout(layer['upd_fn'](agg))     # residual
            H = layer['norm'](H)
        return H


# [V3-3] SharedReconDecoder replaces N separate PerVarReconDecoder
class SharedReconDecoder(nn.Module):
    """Shared decoder for all variables."""
    def __init__(self, d, out_len, dropout=0.0):
        super().__init__()
        self.out_len = out_len
        self.expand = nn.Linear(d, d * out_len)
    ...
    def forward(self, c):
        # c: [B*N, d]
        h = self.expand(c).view(-1, self.out_len, c.size(-1))  # [B*N, out_len, d]
        h = h.transpose(1, 2)  # [B*N, d, out_len]
        h = h + F.gelu(self.norm1(self.conv1(h)))
        h = h + F.gelu(self.norm2(self.conv2(h)))
        h = h.transpose(1, 2)  # [B*N, out_len, d]
        return self.out_proj(h).squeeze(-1)  # [B*N, out_len]
...
# Main model
# ============================================================
class OracleAD3DCausalEffectV3(nn.Module):
    def __init__(self, N: int, L: int, tau_max: int, d: int, heads: int,
        ...
        # [V3-1] Shared temporal encoder instead of N separate PerVarEncoders
        self.shared_encoder = SharedTemporalEncoder(d, conv_layers, dropout)

        # [V3-4] Variable identity embedding
        self.var_embed = nn.Parameter(torch.randn(N, d) * 0.02)

        # [V3-2] Causal GNN layer
        self.gnn = CausalGNNLayer(d, gnn_layers, dropout)

        self.mhsa = nn.MultiheadAttention(d, heads, batch_first=True, dropout=dropout)

        # [V3-3] Shared reconstruction decoder instead of N separate PerVarReconDecoders
        self.shared_decoder = SharedReconDecoder(d, L - 1, dropout)
        ...
        # [V3-5] Edge-specific feature: edge_value from (source, target) pair
        # Instead of Linear(C[src]) -> tgt, use MLP(C[src] || C[tgt]) -> scalar
        # This gives much stronger edge differentiation
        self.edge_src_proj = nn.Linear(d, d // 2)
        self.edge_tgt_proj = nn.Linear(d, d // 2)
        self.edge_value_mlp = nn.Sequential(
            nn.Linear(d, d),
            nn.GELU(),
            nn.Linear(d, 1),
        )
    ...
    def forward(self, X, mask_tau=None, mask_var=None, mask_fill_value=0.0):
        ...
        for tau in range(1, self.tau_max + 1):
            ...
            x_slice = X[:, start:end, :]  # [B, win, N]

            # Masking (for intervention)
            if (mask_tau is not None) and (mask_var is not None):
                if tau == int(mask_tau):
                    x_slice = x_slice.clone()
                    x_slice[:, :, int(mask_var)] = float(mask_fill_value)

            win = end - start
            # Shared encoding: reshape [B, win, N] -> [B*N, win, 1]
            x_flat = x_slice.permute(0, 2, 1).reshape(B * N, win, 1)
            # Expand var_embed for batch: [N, d] -> [B*N, d]
            ve = self.var_embed.unsqueeze(0).expand(B, -1, -1).reshape(B * N, self.d)
            c_flat = self.shared_encoder(x_flat, var_embed=ve)  # [B*N, d]
            C_tau = c_flat.reshape(B, N, self.d)  # [B, N, d]

            # GNN message passing along causal graph
            # If a variable is masked for intervention, block its outgoing messages
            # so GNN cannot "leak" the masked info to other variables
            ...
            C_tau = self.gnn(C_tau, adj_tau, mask_var=gnn_mask)  # [B, N, d]

            # MHSA with causal mask
            causal_mask = self._causal_attn_mask(tau)
            A_tau, _ = self.mhsa(C_tau, C_tau, C_tau, attn_mask=causal_mask, need_weights=False)
        ...
        c_flat = C_recon.reshape(B * N, self.d)
        recon_flat = self.shared_decoder(c_flat)  # [B*N, L-1]
        recon = recon_flat.reshape(B, N, self.L - 1).permute(0, 2, 1)  # [B, L-1, N]

        # Prediction — [V3-5] edge-specific features from (src, tgt) pairs
        local_delta = self._compute_dynamic_delta(C_all)
        pred_weights = self.get_pred_weights(local_delta=local_delta)

        # C_all: [B, tau, N, d]
        B_sz, tau_sz, N_sz, d_sz = C_all.shape
        src_feat = self.edge_src_proj(C_all)  # [B, tau, N_src, d//2]
        tgt_feat = self.edge_tgt_proj(C_all)  # [B, tau, N_tgt, d//2]
        # Expand: src [B, tau, N_src, 1, d//2] × tgt [B, tau, 1, N_tgt, d//2]
        src_exp = src_feat.unsqueeze(3).expand(-1, -1, -1, N_sz, -1)
        tgt_exp = tgt_feat.unsqueeze(2).expand(-1, -1, N_sz, -1, -1)
        pair_feat = torch.cat([src_exp, tgt_exp], dim=-1)  # [B, tau, N_src, N_tgt, d]
        edge_value = self.edge_value_mlp(pair_feat).squeeze(-1)  # [B, tau, N_src, N_tgt]
...
# ============================================================
def main():
    ap = argparse.ArgumentParser(description="OracleAD causal v3 runner")
    ...
    # [V3] new architecture params
    ap.add_argument("--conv_layers", type=int, default=3,
                    help="Number of conv layers in SharedTemporalEncoder.")
    ap.add_argument("--gnn_layers",  type=int, default=2,
                    help="Number of GNN message-passing layers in CausalGNNLayer.")
    ...
    ap.add_argument("--tb_root",      type=str, default="runs/tensorboard/oraclead_causal_v3")
    ...
    ap.add_argument("--out_dir",       type=str, default="runs/oraclead_causal_v3")
    ...
    for f in files:
        ...
        print(
            ...
            f"conv_layers={args.conv_layers} gnn_layers={args.gnn_layers} "
        ...
        for seed in seeds:
            ...
            model = OracleAD3DCausalEffectV3(
                N=N,
                L=args.L,
                tau_max=args.tau_max,
                d=args.d,
