#!/usr/bin/env bash
# Phase 1 score-component ablation: individual P_only, C_only, G_only.
# Uses the same best-epoch-only rescore pipeline as v3, but with only the
# individual score fusions (no re-normalization needed since ranking metrics
# are invariant to monotonic transforms of a single score).
#
# Layout / GPU sharding mirrors v3 for wall-clock parity.
#
# Usage:
#   nohup bash scripts/run_rescore_v4.sh > logs/rescore_v4_master.log 2>&1 &
...
launch_smd_partition() {
    ...
    local logf="logs/rescore_v4_${out}_gpu${gpu}.log"
...
echo "[$(date +'%F %T')] rescore v4 START (Phase 1: individual scores $FUSIONS)"

# ============================================================
# Phase 1a: PSM × 3 variants (fast, 1 entity 4 seeds → per-seed on 4 GPUs)
# ============================================================
for pair in \
...
echo "[$(date +'%F %T')] PSM DONE. Starting SWaT."

# ============================================================
# Phase 1b: SWaT × 3 variants (large test, per-seed sharding)
# ============================================================
for pair in \
...
# ============================================================
# Phase 1c: SMD × 3 variants (28 entities, partitioned on 4 GPUs)
# ============================================================
for pair in \
...
echo "[$(date +'%F %T')] ALL RESCORE V4 (Phase 1) DONE"
