#!/usr/bin/env bash
# Standalone SMD no_all_4 config run.
#
# Purpose: fallback if the addition to run_regab_smd.sh isn't picked up by the
# currently running master orchestrator (bash may have cached the script).
# Runs the single config where all four auxiliary regularizers are disabled:
#   W_GATE = W_GRAPH = W_LAGMONO = W_INV = 0.
#
# Usage:
#   nohup bash scripts/run_regab_smd_no_all_4.sh \
#       > logs/regab_smd_no_all_4_master.log 2>&1 &
set -u
...
launch_partition() {
    local gpu="$1" ents="$2"
    local out="${OUT_ROOT}/smd_regab_no_all_4_${DATE}"
    local logf="logs/regab_smd_no_all_4_gpu${gpu}.log"
    mkdir -p "$out"
    echo "[launch] GPU $gpu  no_all_4  entities=$(echo "$ents" | awk -F',' '{print NF}')"
...
echo "[$(date +'%F %T')] SMD no_all_4 START"
...
echo "[$(date +'%F %T')] SMD no_all_4 DONE"
echo "Result: ${OUT_ROOT}/smd_regab_no_all_4_${DATE}"
