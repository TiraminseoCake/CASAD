#!/usr/bin/env bash
# SMD core-loss ablation: L_crs disabled (W_CRS=0).
set -u
cd "$(dirname "$0")/.."
...
launch_partition() {
    local gpu="$1" ents="$2"
    local out="${OUT_ROOT}/smd_regab_no_crs_${DATE}"
    local logf="logs/regab_smd_no_crs_gpu${gpu}.log"
    mkdir -p "$out"
    echo "[launch] GPU $gpu  no_crs  entities=$(echo "$ents" | awk -F',' '{print NF}')"
    CUDA_VISIBLE_DEVICES="$gpu" nohup "$PYTHON" -u main.py \
        ...
        SOLVER.MAX_EPOCH 80 \
        PICAAD.LOSS.W_CRS 0.0 \
        > "$logf" 2>&1 &
}

echo "[$(date +'%F %T')] SMD no_crs START"
